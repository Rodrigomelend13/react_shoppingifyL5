...
import { findActiveList, updateList } from "../api/List";
...
function MainRightPanel() {
  ...
  async function handleChangeItemListName() {
    ...
    const updatedName = changedName.current?.value.replace(/'/g, "&apos;");

    await updateList(activeList.id, updatedName, localStorage.getItem("user_id"));
    refetch();
    closeModal();
  }
