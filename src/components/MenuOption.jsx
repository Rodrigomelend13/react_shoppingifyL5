...
import { MdFormatListBulleted, MdInsertChartOutlined, MdReplay } from "react-icons/md";
...
function MenuOption(props) {
  ...
  var route = isMainMenuOption ? "/" : isHistoryMenuOption ? "/history" : "/statistics";

  var tooltipMessage = isMainMenuOption ? "items" : isHistoryMenuOption ? "history" : "statistics";
      ...
      <Link to={route}>
        <div className="flex h-14 w-28">
          <div className="flex items-center space-x-8">
            <div className={highlightIcon ? "w-2 h-full bg-mainYellow rounded-r-lg" : "w-2"}></div>
            <>
              {isMainMenuOption ? (
                <MdFormatListBulleted size={30} color="fullDark"></MdFormatListBulleted>
              ) : isHistoryMenuOption ? (
                <MdReplay size={30} color="fullDark"></MdReplay>
              ) : (
                <MdInsertChartOutlined size={30} color="fullDark"></MdInsertChartOutlined>
              )}
            </>
          </div>
        </div>
